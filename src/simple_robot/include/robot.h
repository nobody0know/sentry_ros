...
namespace robomaster
{
  ...
  private:
    bool ROSInit()
    {
      ...
      chassis_odom_pub_ = nh.advertise<nav_msgs::Odometry>("odom", 10);
    ...
    uint16_t ReceiveDataSolve(uint8_t *frame)
    {
      ...
      else
      {
        ...
        case CHASSIS_ODOM_CMD_ID:
        {
          ...
          if (chassis_odom_info_.vx < 0.1 && chassis_odom_info_.vx > -0.1 && chassis_odom_info_.vy < 0.1 && chassis_odom_info_.vy > -0.1 && chassis_odom_info_.vw < 0.1 && chassis_odom_info_.vw > -0.1)
          {
